...
def splitCont(mwe, startlen, endlen, start_idx, sentence)
	return sentence[0..start_idx-1].join(), sentence[start_idx..start_idx+mwe.length-1].join(), sentence[start_idx+mwe.length..-1].join()
end
...
def proc(pathtocorp, mwelist, consthash)
	# open the corpus
...
def main()
	...
	# 辞書for文
		for type in ["train", "test", "dev"] do
			...
			proc(pathtocorp, mwelist, consthash)
		end
